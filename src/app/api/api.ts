...
const api = {
    ...
    food: { 
        list: async (): Promise<Fastfood[]> => {
            return fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vSsjVwDTTlJGFXe4fsHVVKaHH_rwoYEezikCB1M6n1TW7r8TsE_0fnVIUo-pV6hjw/pub?gid=1990032142&single=true&output=tsv'
            ).then(res => res.text()) 
            .then(text => {
                return text.split('\n').slice(1).map(row => {
